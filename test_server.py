#!/usr/bin/env python3
"""
Test script to verify MCP Task Orchestrator server functionality.
"""
...
async def test_server_startup():
    ...
    try:
        ...
        if process.poll() is None:
            ...
            print(f"❌ Server exited with return code: {process.returncode}")
            if stderr:
...
def main():
    ...
    if not deps_ok:
        print("❌ Some dependencies are missing. Please install requirements:")
    ...
    if server_ok:
        print("🎉 All tests passed! The MCP Task Orchestrator is ready to use.")
        print("\nNext steps:")
