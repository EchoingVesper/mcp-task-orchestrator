...
# All logging must go to stderr to prevent protocol corruption

# Stderr handler for ALL logging levels (since stdout is reserved for MCP protocol)
stderr_handler = logging.StreamHandler(sys.stderr)
stderr_handler.setLevel(logging.DEBUG)  # Capture all logging levels
...
# MCP Prompts handlers (required by MCP protocol)
@app.list_prompts()
async def list_prompts() -> List[types.Prompt]:
    """List available prompts."""
    # Currently, this server doesn't provide any prompts
    # But we need to implement this handler to prevent "Method not found" errors
    return []
...
async def get_prompt(name: str, arguments: Dict[str, str] | None = None) -> types.GetPromptResult:
    ...
    # This server doesn't implement prompts, so we raise an error for any request
    raise ValueError(f"Unknown prompt: {name}")


# MCP Resources handlers (required by MCP protocol)
@app.list_resources()
async def list_resources() -> List[types.Resource]:
    """List available resources."""
    # Currently, this server doesn't provide any resources
    # But we need to implement this handler to prevent "Method not found" errors
    return []
...
async def read_resource(uri: str) -> str:
    ...
    # This server doesn't implement resources, so we raise an error for any request
