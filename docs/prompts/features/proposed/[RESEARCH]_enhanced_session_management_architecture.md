...
class SessionState(Enum):
    ...
    ACTIVE = "active"         # Currently active session (only one allowed)
    PAUSED = "paused"         # Temporarily inactive but resumable
...
class SessionStateTransitions:
    ALLOWED_TRANSITIONS = {
        ...
        SessionState.ARCHIVED: [SessionState.ACTIVE],  # Can reactivate archived sessions
        SessionState.CANCELLED: [SessionState.ARCHIVED],
        SessionState.ERROR: [SessionState.ACTIVE, SessionState.CANCELLED]  # Recovery paths
    }
```
...
class MarkdownSyncEngine:
    ...
    def parse_user_changes(self, markdown_content: str, session_id: str) -> List[Change]:
        ...
        notes_section = self.extract_section(markdown_content, "## 📝 Session Notes")
        if notes_section:
